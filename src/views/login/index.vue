        ...
        <div class="login">
          <van-form @submit="login" class="formData" ref="loginForms">
            <van-cell-group inset>
              ...
              <van-field v-model="user.password" type="password" name="验证码" label="验证码" placeholder="验证码"
        ...
        <div class="register">
              <van-form class="formData">
                <van-cell-group inset>
                  ...
                  <van-field v-model="user.password" type="password" name="密码" label="密码" placeholder="确认密码"
                    :rules="[{ required: true, message: '请填写密码' }]" />
                </van-cell-group>
                <div style="margin: 16px;">
                  <van-button round block type="primary" native-type="submit">
...
const rules:any = ref({
  username:[
   { required: true, message:"账号只可包含字母、数字、下划线和减号 长度在3到16之间",trigger:"change"},
   { pattern:/^[a-zA-Z0-9_-]{3,16}$/,trigger:'change'}
  ],
  password:[
   { required: true, message:"密码必须包含一个大写字母、小写字母、数字 6-15位",trigger:"change"},
